...
steps:
  ...
  - name: "スキーマ更新の実行"
    ...
    params:
      version: "v1.0.1"
      new_cel_expression: "this.user_plan == 'free' ? this.message.size() <= 200 : this.message.size() <= 300"
  ...
  - name: "セキュリティ検証: 悪意のあるバイパス"
    ...
    description: "クライアント側で user_plan を premium に偽装して 250 文字送る"
    params:
      message_length: 250
      fake_user_plan: "premium" 
    expected:
...
* FE, BE, ISR, DB を含む全スタックを Docker 上に展開。
