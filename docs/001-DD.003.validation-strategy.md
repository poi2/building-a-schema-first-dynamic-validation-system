...
* **注意**: BFF はバリデーションを行わないため、不正なリクエストは BE で最終的に検証される。Early Reject のメリットはないが、PoC としては許容範囲。

* **将来的な拡張案**: リクエストに `user_fetched_at` を追加し、それが古い場合は BFF で DB から refetch してバリデーションを行う方法も検討可能。
