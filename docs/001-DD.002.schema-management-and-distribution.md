...
* **初期化**:
  1. 起動時に ISR から `GetLatestPatch(major_minor: CELO_SCHEMA_TARGET)` を呼び出し。
...
* **初期化**:
  1. 起動時に BFF からスキーマを取得。
  2. 取得できなかった場合、コンテナイメージに同梱された proto をフォールバックとして使用。
* **監視**: 1分間隔で BFF をポーリング。
* **プロアクティブ通知**: BFF が WebSocket/SSE で更新通知を送信し、FE は即座にスキーマを再取得。
...
* **FE/BFF/BE のコンテナイメージ**:
  1. ビルド時に `buf build` を実行し、`descriptor.bin` をイメージに含める。
  2. ISR から最新 Patch を受け取れた場合は、メモリ上のバリデーターを更新。
  3. ISR から一度も受け取れなかった場合は、イメージに同梱された proto を使用。
